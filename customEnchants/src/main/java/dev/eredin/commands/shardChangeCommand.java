...
public class shardChangeCommand implements CommandExecutor {
            ...
            if (args.length >= 3){
                if (Bukkit.getPlayer(args[1]) != null && Bukkit.getPlayer(args[1]).isOnline()) {
                    ...
                    if (args[0].equalsIgnoreCase("add")) {
                        ...
                        try {
                            x.shards += Integer.parseInt(args[2]);
                            sender.sendMessage(ChatColor.LIGHT_PURPLE+"Successfully added "+args[2]+" shards to "+ args[1]);
                    ...
                    if (args[0].equalsIgnoreCase("set")) {
                        ...
                        try {
                            x.shards = Integer.parseInt(args[2]);
                            sender.sendMessage(ChatColor.LIGHT_PURPLE+"Successfully set "+args[1]+"'s shards to "+ args[2]);
                            dataManager.getData(targetUser.getUniqueId()).updatePickLore(targetUser);
                            return true;
                        } catch (Exception e) {
                            sender.sendMessage(ChatColor.RED + "You did not enter a valid number!");
